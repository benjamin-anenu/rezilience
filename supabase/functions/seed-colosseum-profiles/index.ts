...
interface CohortData {
  ...
  description?: string;
  projects?: StartupRecord[];
}

interface ColosseumData {
  metadata: Record<string, unknown>;
  accelerator_cohort_1_renaissance: CohortData;
  accelerator_cohort_2_radar: CohortData;
  accelerator_cohort_3_breakout: CohortData;
  accelerator_cohort_4_cypherpunk: CohortData;
  hackathon_winners_not_in_accelerator: CohortData;
  summary_stats: Record<string, unknown>;
}

// Helper to map cohort key to discovery source
function getDiscoverySource(cohortKey: string, hackathonName?: string, prize?: string): string {
  ...
  if (cohortMap[cohortKey]) {
    return cohortMap[cohortKey];
  }

  // For hackathon winners not in accelerator
  if (hackathonName) {
    const prizeInfo = prize ? ` - ${prize}` : "";
    return `Colosseum ${hackathonName}${prizeInfo}`;
  }

  return "Colosseum Hackathon";
}

// Transform a startup record to claimed_profiles format
function transformStartup(
  startup: StartupRecord,
  cohortKey: string
): Record<string, unknown> {
  // Handle program_id: set to NULL if TBD or N/A
  const programId =
    startup.program_id &&
    !startup.program_id.includes("TBD") &&
    startup.program_id !== "N/A"
      ? startup.program_id
      : null;

  // Handle github_url: set to NULL if "private"
  const githubOrgUrl =
    startup.github_url && startup.github_url !== "private"
      ? startup.github_url
      : null;

  // Handle x_handle: strip @ prefix
  const xUsername = startup.x_handle
    ? startup.x_handle.replace(/^@/, "")
    : null;
  ...
  return {
    ...
    country: startup.location || null,
    github_contributors: startup.contributors_approx || 0,
    discovery_source: getDiscoverySource(
      cohortKey,
      startup.hackathon,
      startup.prize
    ),
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Colosseum startups data (embedded to avoid file system access issues)
    ...
    // Collect all startups from all cohorts
    const allStartups: { startup: StartupRecord; cohortKey: string }[] = [];

    // Process accelerator cohorts
    const acceleratorCohorts = [
      "accelerator_cohort_1_renaissance",
      "accelerator_cohort_2_radar",
      "accelerator_cohort_3_breakout",
      "accelerator_cohort_4_cypherpunk",
    ...
    for (const cohortKey of acceleratorCohorts) {
      const cohort = colosseumData[cohortKey];
      if (cohort?.startups) {
        for (const startup of cohort.startups) {
          allStartups.push({ startup, cohortKey });
        }
      }
    }

    // Process hackathon winners
    const hackathonWinners = colosseumData.hackathon_winners_not_in_accelerator;
    if (hackathonWinners?.projects) {
      for (const startup of hackathonWinners.projects) {
        allStartups.push({
          startup,
          cohortKey: "hackathon_winners_not_in_accelerator",
        });
      }
    }

    console.log(`Processing ${allStartups.length} startups...`);

    // Transform all startups to claimed_profiles format
    const profiles = allStartups.map(({ startup, cohortKey }) =>
      transformStartup(startup, cohortKey)
    );

    // Batch upsert using project_name as the conflict key
    ...
    const result = {
      success: true,
      message: `Successfully seeded ${profiles.length} Colosseum startups`,
      inserted: data?.length || 0,
      startups: profiles.map((p) => p.project_name),
    };
